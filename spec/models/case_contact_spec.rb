...
RSpec.describe CaseContact, type: :model do
  ...
  it "can't be updated when occured_at is after the last day of the month in the quarter that the case contact was created" do
    case_contact = build_stubbed(:case_contact, occurred_at: Time.zone.now - 1.year)
    expect(case_contact).to_not be_valid
  ...
  describe "scopes" do
    describe ".contact_type" do
      it "returns case contacts filtered by contact type id" do
        group = build_stubbed(:contact_type_group)
        youth_type = build(:contact_type, name: "Youth", contact_type_group: group)
        supervisor_type = build(:contact_type, name: "Supervisor", contact_type_group: group)
        parent_type = build(:contact_type, name: "Parent", contact_type_group: group)
        ...
        build(:case_contact, contact_types: [parent_type])
    ...
    describe ".has_transitioned" do
      let(:casa_case_1) { build(:casa_case, transition_aged_youth: true) }
      let(:casa_case_2) { build(:casa_case, transition_aged_youth: false) }
      ...
      context "with yes option" do
        it "returns case contacts filtered by contact made option" do
          ...
          build_stubbed(:case_contact, {casa_case: casa_case_2})
      ...
      context "with no option" do
        it "returns case contacts filtered by contact made option" do
          build_stubbed(:case_contact, {casa_case: casa_case_1})
