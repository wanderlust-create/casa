...
RSpec.describe "case_contact_reports/index", type: :system do
  ...
  it "filters report by date and selected contact type", js: true do
    ...
    court = build(:contact_type, name: "Court", contact_type_group: contact_type_group)
    school = build(:contact_type, name: "School", contact_type_group: contact_type_group)
    ...
    excluded_by_date = build_stubbed(:case_contact, occurred_at: 40.days.ago, contact_types: [court], notes: "Excluded by date")
    excluded_by_contact_type = build_stubbed(:case_contact, occurred_at: 20.days.ago, contact_types: [school], notes: "Excluded by Contact Type")
  ...
  it "filters report by contact type group", js: true do
    ...
    contact_type_group = build(:contact_type_group)
    court = build(:contact_type, name: "Court", contact_type_group: contact_type_group)
    contact1 = create(:case_contact, occurred_at: Date.yesterday, contact_types: [court], notes: "Case Contact 1")

    excluded_contact_type_group = build_stubbed(:contact_type_group)
    school = build_stubbed(:contact_type, name: "School", contact_type_group: excluded_contact_type_group)
    excluded_by_contact_type_group = build_stubbed(:case_contact, occurred_at: Date.yesterday, contact_types: [school], notes: "Excluded by Contact Type")
