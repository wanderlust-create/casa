...
RSpec.describe CaseContactsHelper do
  ...
  describe "#set_contact_made_false" do
    ...
    describe "#create_by_link" do
    it "renders creator name when user is a volunteer" do
      current_user = create(:volunteer)
      casa_case = create(:casa_case)
      case_contact = build(:case_contact, duration_minutes: 0)
      allow(helper).to receive(:current_user).and_return(current_user)

      expect(helper.created_by_link(case_contact)).to eq(case_contact.creator.display_name)
    end
    ...
    it "renders back link to home page when user is a supervisor" do
      ...
      expect(helper.created_by_link(case_contact)).to eq(casa_case_path(casa_case))
    end
